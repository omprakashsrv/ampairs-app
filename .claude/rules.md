...
- Load all user-visible strings from Android string resources (`strings.xml`) via `context.getString(R.string.xxx)` — never hardcode UI text in Kotlin source files; place strings in the module's own `androidMain/res/values/strings.xml`
