...
/**
 * Applies an approved `customer.phone` [Candidate] by rewriting the customer's `phone` through
...
@AiOpsExecutorKey("customer.phone")
class CustomerPhoneExecutor(
    private val customerRepository: CustomerRepository,
) : AiOpsExecutor {

    override val capabilityKey: String = CustomerPhoneCapability.KEY

    override suspend fun apply(candidate: Candidate, finding: Finding): ExecResult {
        val target = candidate.after
        if (target.isNullOrBlank()) return ExecResult(false, "no target phone")
        val customer = customerRepository.getCustomer(finding.entityId)
            ?: return ExecResult(false, "customer ${finding.entityId} not found")
        if (customer.phone == target) return ExecResult(true, "already \"$target\"")

        return customerRepository.updateCustomer(customer.copy(phone = target)).fold(
            onSuccess = { ExecResult(true, "customer phone → \"$target\"") },
