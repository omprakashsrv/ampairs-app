 ...
 * populates the dropdown from live, already-synced workspace data — value = uid, label = name.
 */
@Inject
...
class CustomerTypeOptionProvider(
    ...
    override fun options(): Flow<List<FieldOption>> =
        repository.observeCustomerTypes().map { types -> types.map { FieldOption(it.uid, it.name) } }
}
...
class CustomerGroupOptionProvider(
    ...
    override fun options(): Flow<List<FieldOption>> =
        repository.observeCustomerGroups().map { groups -> groups.map { FieldOption(it.uid, it.name) } }
}
