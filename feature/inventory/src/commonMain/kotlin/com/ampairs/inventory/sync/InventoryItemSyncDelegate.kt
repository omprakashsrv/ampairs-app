...
class InventoryItemSyncDelegate(
    ...
    private suspend fun pull(batchSize: Int = 100): Result<Int> = try {
        val lastSync = syncStateDao.getLastSyncedAtIso(SyncEntity.INVENTORY) ?: ""
        var totalSynced = 0
        var currentPage = 0
        var maxTime = ""
        do {
            val pageResponse = api.getItems(currentPage, batchSize, lastSync.ifBlank { null })
        ...
        if (maxTime.isNotBlank()) syncStateDao.setLastSyncedAtIso(SyncEntity.INVENTORY, maxTime)
