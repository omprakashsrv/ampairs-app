...
/**
 * Adds indexes on the columns used by the product list filter (brand, category, sub-category) so
 * multi-select filtering stays fast as the product table grows. Index names match the [Index] names
 * declared on [com.ampairs.product.db.entity.ProductEntity] so Room's schema validation passes.
 */
