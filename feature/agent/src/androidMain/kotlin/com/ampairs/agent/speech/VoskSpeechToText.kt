...
/**
 * Android offline STT via **Vosk (Kaldi)** — a third STT adapter alongside the device recognizer and
 * Whisper. Uses the `vosk-android` AAR (matched per-ABI native `.so` + JNA), so the desktop arm64
 * native problems don't apply here. The model (a directory) downloads on demand through
 * [VoskModelRegistry] over the shared archive-aware `ModelManager`.
 *
 * Recognition mirrors the desktop engine: feed live 16 kHz mono PCM (from the shared [AudioCapture],
 * converted to 16-bit) to `Recognizer.acceptWaveForm`, emit [SttEvent.Partial] from `getPartialResult()`
 * and the first non-blank `getResult()` (silence boundary) as the [SttEvent.Final]. `languageTag` is
 * informational — the Vosk model is language-fixed by its directory.
 */
