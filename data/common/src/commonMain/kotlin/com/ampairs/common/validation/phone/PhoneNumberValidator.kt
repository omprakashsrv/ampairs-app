package com.ampairs.common.validation.phone

import com.ampairs.common.validation.Invalid
import com.ampairs.common.validation.Valid
import com.ampairs.common.validation.ValidationResult
import com.ampairs.common.validation.Validator

class PhoneNumberValidator : Validator<String?, String> {

    companion object {
        // Enhanced phone validation patterns based on libphonenumber concepts
        private val INDIAN_MOBILE_REGEX = Regex("^[6789]\\d{9}\$")
        private val INDIAN_LANDLINE_REGEX = Regex("^0[1-9]\\d{7,9}\$")
        private val INTERNATIONAL_REGEX = Regex("^\\+[1-9]\\d{1,14}\$")

        // Common country codes for validation
        private val VALID_COUNTRY_CODES = setOf(
            1, 7, 20, 27, 30, 31, 32, 33, 34, 36, 39, 40, 41, 43, 44, 45, 46, 47, 48, 49,
            51, 52, 53, 54, 55, 56, 57, 58, 60, 61, 62, 63, 64, 65, 66, 81, 82, 84, 86,
            90, 91, 92, 93, 94, 95, 98, 212, 213, 216, 218, 220, 221, 222, 223, 224, 225,
            226, 227, 228, 229, 230, 231, 232, 233, 234, 235, 236, 237, 238, 239, 240, 241,
            242, 243, 244, 245, 246, 247, 248, 249, 250, 251, 252, 253, 254, 255, 256, 257,
            258, 260, 261, 262, 263, 264, 265, 266, 267, 268, 269, 290, 291, 297, 298, 299,
            350, 351, 352, 353, 354, 355, 356, 357, 358, 359, 370, 371, 372, 373, 374, 375,
            376, 377, 378, 380, 381, 382, 383, 385, 386, 387, 389, 420, 421, 423, 500, 501,
            502, 503, 504, 505, 506, 507, 508, 509, 590, 591, 592, 593, 594, 595, 596, 597,
            598, 599, 670, 672, 673, 674, 675, 676, 677, 678, 679, 680, 681, 682, 683, 684,
            685, 686, 687, 688, 689, 690, 691, 692, 850, 852, 853, 855, 856, 880, 886, 960,
            961, 962, 963, 964, 965, 966, 967, 968, 970, 971, 972, 973, 974, 975, 976, 977,
            992, 993, 994, 995, 996, 998
        )
    }

    override fun validate(input: String?): ValidationResult<String> {
        if (input == null) return Invalid(PhoneNumberValidationError.InputIsNull)

        val cleanInput = input.trim()
        if (cleanInput.isEmpty()) return Invalid(PhoneNumberValidationError.Empty)

        // Check for international format
        if (cleanInput.startsWith("+")) {
            return validateInternationalNumber(cleanInput)
        }

        // Check for Indian mobile number (default country)
        if (cleanInput.matches(INDIAN_MOBILE_REGEX)) {
            return Valid(cleanInput)
        }

        // Check for Indian landline number
        if (cleanInput.matches(INDIAN_LANDLINE_REGEX)) {
            return Valid(cleanInput)
        }

        return Invalid(PhoneNumberValidationError.InvalidFormat)
    }

    private fun validateInternationalNumber(number: String): ValidationResult<String> {
        if (!number.matches(INTERNATIONAL_REGEX)) {
            return Invalid(PhoneNumberValidationError.InvalidFormat)
        }

        // Extract country code
        val countryCodeMatch = Regex("^\\+(\\d{1,3})").find(number)
        if (countryCodeMatch == null) {
            return Invalid(PhoneNumberValidationError.InvalidCountryCode)
        }

        val countryCode = countryCodeMatch.groupValues[1].toIntOrNull()
        if (countryCode == null || countryCode !in VALID_COUNTRY_CODES) {
            return Invalid(PhoneNumberValidationError.InvalidCountryCode)
        }

        // Validate number length (total should be between 7-15 digits including country code)
        val totalDigits = number.substring(1).length
        if (totalDigits < 7 || totalDigits > 15) {
            return Invalid(PhoneNumberValidationError.InvalidLength)
        }

        return Valid(number)
    }
}