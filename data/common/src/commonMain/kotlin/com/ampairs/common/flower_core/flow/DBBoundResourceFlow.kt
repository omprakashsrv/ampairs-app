...
package com.ampairs.common.flower_core.flow

import com.ampairs.common.flower_core.ApiEmptyResponse
import com.ampairs.common.flower_core.ApiErrorResponse
import com.ampairs.common.flower_core.ApiResponse
import com.ampairs.common.flower_core.ApiSuccessResponse
...
inline fun <DB, REMOTE> dbBoundResourceFlow(
    ...
    crossinline onNetworkRequestFailed: (errorMessage: String, errorCode: String) -> Unit = { _: String, _: String -> },
) = flow<Resource<DB>> {
    ...
    if (shouldMakeNetworkRequest(localData)) {
        ...
        makeNetworkRequest().collect { apiResponse ->
            when (apiResponse) {
                ...
                is ApiErrorResponse -> {
                    onNetworkRequestFailed(apiResponse.errorMessage, apiResponse.errorCode)
                    emitAll(
                        fetchFromLocal().map { dbData ->
                            Resource.error(
